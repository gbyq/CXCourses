...
public class LocalCourseDAo {
    ...
    public LocalCourse find(String name) {
        db = helper.getWritableDatabase();
        Cursor cursor = db.rawQuery("select name,local_path,is_finish from local_course where name = ?",
                new String[]{name});
        if (cursor.moveToNext()) {
            return new LocalCourse(cursor.getString(cursor.getColumnIndex("name")),
    ...
    public List<LocalCourse> getScrollData() {
        ...
        while (cursor.moveToNext()) {
            localCourses.add(new LocalCourse(cursor.getString(cursor.getColumnIndex("name")),
                    cursor.getString(cursor.getColumnIndex("local_path")),
                    cursor.getInt(cursor.getColumnIndex("is_finish"))));
        }
        return localCourses;
